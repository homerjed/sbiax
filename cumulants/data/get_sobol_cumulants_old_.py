...
ALL_LATIN_PARAMS = np.loadtxt(LATINS_DIR / "BSQ_params.txt")


def get_pdf_derivative_filename_template(derivative_p_m_name, redshift, realisation, R_i):
...
def get_pdf_filename_template(redshift, R_i, realisation):
    filename = 'matter_PDF_linbins_BSQ_z_{:.2f}_extraRind_{}_{}.dat'.format(
        redshift, R_i, str(realisation)
    )
    return filename
...
) -> tuple[Float[Array, "d"], Int[Array, "c"]]:
    n_bins_pdf = pdf.size
...
def get_fiducials_fixed_lengths(redshift, cdf_cut_lims):
    ...
    for _, realisation_path_folder_number in zip(
        bar := trange(
            N_FIDUCIAL_REALISATIONS, 
            colour="blue", 
            desc="Fiducials (PDF lengths)"
        ),
        ...
        for i_z, z in enumerate(REDSHIFTS):  
            ...
            for i_r, radius in enumerate(R_numbers):
                ...
                # Grab physical scale index (NOTE: wrong; all_R_numbers == radius)
                ...
                realisation_path = realisation_dir / get_pdf_filename_template(
                    redshift=z, R_i=idx, realisation=realisation_folder_number
                ) 
...
def get_fiducials_latins_derivatives_bulk_pdf(
    ...
    # Dimension from summing all cuts at redshift
    ...
    fiducial_pdfs = np.zeros((N_FIDUCIAL_REALISATIONS, len(redshifts), all_scales_for_z_dim))
        ...
        for i_z, z in enumerate(redshifts):  # ONLY REDSHIFT zero
            ...
            for i_r, radius in enumerate(R_numbers):
                ...
                try:
                    ...
                    cut_pdf = pdf[cdf_cut_idx][:z_R_length] # Trim
            ...
            if not bad:
                fiducial_pdfs[realisation_idx, i_z] = np.concatenate(_pdfs) 
                pass
    ...
    derivatives_pdfs = np.zeros((N_DERIVATIVE_REALISATIONS, len(redshifts), ALPHA.size, all_scales_for_z_dim))
    ...
    for i_p in range(ALPHA.size):
        ...
        for realisation, realisation_path_folder_number in zip(
            trange(
                ...
                desc=PARAMETER_STRINGS[i_p][1:-1] + " (Bulk PDFs)",
                colour="green"
            ),
        ...
        derivatives_pdfs[:, :, i_p, :] = (dummy_pdfs[..., 0] - dummy_pdfs[..., 1]) / DPARAMS[i_p]

    bad_realisations = list(set(bad_realisations))
    for bad_idx in bad_realisations:
        derivatives_pdfs = np.delete(derivatives_pdfs, bad_idx, axis=0)

    return np.squeeze(fiducial_pdfs, axis=1), np.squeeze(derivatives_pdfs, axis=1)
...
def get_fiducials_latins_derivatives_cumulants(
    ...
    fiducial_cumulants = np.zeros((N_FIDUCIAL_REALISATIONS, len(redshifts), len(scales), n_cumulants))
    fiducial_vars = np.zeros((N_FIDUCIAL_REALISATIONS, len(redshifts), len(scales), 1))
    ...
    for realisation_idx, realisation_path_folder_number in zip(
        ...
        for i_z, z in enumerate(redshifts): 
            ...
            for i_r, radius in enumerate(scale_numbers):
                ...
                # Grab physical scale index
                radius_index = np.squeeze(np.argwhere(scale_numbers == radius))
                ...
                try:
                    ...
                    assert not all(cumulants_R_z == 0)
        ...
        bar.set_description("Fiducials k_n: cut={}".format(bulk_or_tails))

    bad_realisations = list(set(bad_realisations))
    print("BAD REALISATIONS k_n (fiducials):", bad_realisations)
    for bad_idx in bad_realisations:
        fiducial_cumulants = np.delete(fiducial_cumulants, bad_idx, axis=0)
        fiducial_vars = np.delete(fiducial_vars, bad_idx, axis=0) # Delete before averaging
    del bad_realisations

    fiducial_vars = np.mean(fiducial_vars, axis=0, keepdims=True)

    if FIDUCIAL_REDUCE:
        fiducial_cumulants = fiducial_cumulants / (fiducial_vars + 1e-8)

    # Create subplot grid: rows = scales, cols = cumulants
    fig, axes = plt.subplots(
        len(scales), n_cumulants,
        figsize=(4 * n_cumulants, 3 * len(scales)),
        sharex=False, sharey=False
    )

    # Ensure axes is 2D array even if one dim is 1
    axes = np.atleast_2d(axes)

    # Loop over scales and cumulants
    for i_scale, scale in enumerate(scales):
        for j_cum in range(n_cumulants):
            ax = axes[i_scale, j_cum]
            # Plot a histogram for each redshift
            for k_z, z in enumerate(redshifts):
                data = fiducial_cumulants[:, k_z, i_scale, j_cum]
                ax.hist(data, bins="auto", alpha=0.5, label=f"z={z}")
            
            ax.set_title(f"Scale={scale}, Cumulant={j_cum}")
            ax.legend()

    fig.tight_layout()
    plt.savefig("fiducial_cumulants_sobol.png")
    plt.close()
    ...
    cumulants = np.zeros((N_LATIN_REALISATIONS, len(redshifts), len(scales), n_cumulants))
    parameters = np.zeros((N_LATIN_REALISATIONS, ALPHA.size))

    bad_realisations = []
    for realisation_idx, realisation_path_folder_number in zip(
        bar := trange(
            N_LATIN_REALISATIONS, 
        ...
        parameters[realisation_idx] = ALL_LATIN_PARAMS[realisation_folder_number]
        ...
        for i_z, z in enumerate(redshifts): 
            ...
            for i_r, radius in enumerate(scale_numbers):
                ...
                realisation_dir = LATINS_DIR / str(realisation_folder_number) 
                realisation_path = realisation_dir / get_pdf_filename_template(
                    redshift=z, 
                    R_i=idx, 
                    realisation=realisation_folder_number
        ...
        bar.set_description("Latins k_n: cut={}".format(bulk_or_tails))

    bad_realisations = list(set(bad_realisations))
    print("BAD REALISATIONS k_n (latins):", bad_realisations)
    for bad_idx in bad_realisations:
        cumulants = np.delete(cumulants, bad_idx, axis=0)
        parameters = np.delete(parameters, bad_idx, axis=0)
    del bad_realisations

    if FIDUCIAL_REDUCE:
        cumulants = cumulants / (fiducial_vars + 1e-8)
                ...
                for i_z, redshift in enumerate(redshifts):
                    ...
                    for R_i, R in enumerate(scales):
                        ...
                        try:
                            ...
                            dummy[realisation_idx, i_z, R_i, :, pm] = cumulants_R_z
    ...
    bad_realisations = list(set(bad_realisations))
    print("BAD REALISATIONS k_n (derivatives):", bad_realisations)
    for bad_idx in bad_realisations:
        derivatives = np.delete(derivatives, bad_idx, axis=0)
    del bad_realisations

    if FIDUCIAL_REDUCE:
        derivatives = derivatives / (fiducial_vars[:, :, jnp.newaxis, ...] + 1e-8)
    
    # Flatten for fisher forecasting
    fiducial_cumulants = np.reshape(fiducial_cumulants.squeeze(), (len(fiducial_cumulants), -1))
    cumulants = np.reshape(cumulants.squeeze(), (len(cumulants), -1))
    derivatives = np.reshape(derivatives.squeeze(), (len(derivatives), ALPHA.size, -1))

    print("DATA:", jax.tree.map(lambda a: a.shape, (fiducial_cumulants, cumulants, derivatives, parameters)))
    ...
    *, 
    ...
    def generate_dataset() -> list[np.ndarray]:
        ...
        (
            ...
            cdf_cut_lims=(p_value_min, p_value_max)
        )
        ...
        # If requiring PDFs return dataset for bulk of the PDF (not cumulants of the bulk) NOTE: check this...
        ...
        if pdfs:
            ...
            try:
                ...
                pdf_dataset = Dataset(
                    ...
                    data=jnp.zeros((len(latin_parameters), data_dim_pdfs)), # NOTE: Dummy array
                    parameters=jnp.asarray(latin_parameters),
                ...
                # PDFs is now the return dataset
