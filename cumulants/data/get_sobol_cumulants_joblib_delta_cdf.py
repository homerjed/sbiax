...
def cut_pdf_to_cumulants(
    ...
    *,
    ...
    if not np.all(deltas >= -1.01):
        raise ValueError(f"Deltas: some values not greater than -1")
...
def _fiducial_cumulants_row(
    ...
    for R in scale_numbers:
        try:
            ...
            cdf, cut_idx = _get_cdf_of_pdf(pdf, dbins, CUTS[cut_name])

            # Get delta values of CDF's lower/upper bounds
            delta_min = bins[cut_idx.min()]
            delta_max = bins[cut_idx.max()]
...
def _latin_cumulants_row_and_params(
    ...
    for R in scale_numbers:
        try:
            ...
            if realisation_folder_number < 20 and DATASET_TEST:
                ...
                fig.suptitle("PDF and CDF cuts ({})".format(cut_name))

                # ---- PDF plot ----
                ax_pdf.plot(bins, pdf, color="black", lw=1.2, label="PDF full")

                # per-PDF cut (blue)
                ax_pdf.plot(bins[cidx], pdf[cidx], color="blue", lw=1.5, label="perPDFCDF cut")
                ax_pdf.axvline(bins[cidx][0], color="blue", ls=":")
                ax_pdf.axvline(bins[cidx][-1], color="blue", ls=":")

                if cidx.size > 1:
                    ax_pdf.fill_between(bins[cidx], 0.0, pdf[cidx], color="blue", alpha=0.05, zorder=1)

                if cidx.size > 1:
                    x0_b, x1_b = bins[cidx][0], bins[cidx][-1]
                    ax_pdf.axvspan(x0_b, x1_b, color="blue", alpha=0.06, zorder=0)

                # fiducial cut (orange)
                cidx_0 = cuts_cdf_cut_idx_z_R[str(R)]
                ax_pdf.plot(bins[cidx_0], pdf[cidx_0], color="orange", lw=1.5, label="fiducial cut")
                ax_pdf.axvline(bins[cidx_0][0], color="orange", ls=":")
                ax_pdf.axvline(bins[cidx_0][-1], color="orange", ls=":")

                ax_pdf.set_xlabel("δ")
                ax_pdf.set_ylabel("PDF")
                ax_pdf.legend(frameon=False)
                ax_pdf.set_xlim(-1.1, 5.)

                # ---- CDF plot ----
                ax_cdf.plot(bins, cdf, color="black", lw=1.2, label="CDF full", zorder=3)

                # same index highlights on the CDF
                if cidx.size > 1:
                    x0_b, x1_b = bins[cidx][0], bins[cidx][-1]
                    ax_cdf.axvspan(x0_b, x1_b, color="blue", alpha=0.06, zorder=0)

                ax_cdf.plot(bins[cidx], cdf[cidx], color="blue", lw=1.5, label="perPDFCDF cut", zorder=4)
                # shaded area under the blue CDF segment
                if cidx.size > 1:
                    ax_cdf.fill_between(bins[cidx], 0.0, cdf[cidx], color="blue", alpha=0.05, zorder=1)

                ax_cdf.plot(bins[cidx_0], cdf[cidx_0], color="orange", lw=1.5, label="fiducial cut", zorder=5, alpha=0.9)
                ax_cdf.axvline(bins[cidx][0], color="blue", ls=":", zorder=2)
                ax_cdf.axvline(bins[cidx][-1], color="blue", ls=":", zorder=2)
                ax_cdf.axvline(bins[cidx_0][0], color="orange", ls=":", zorder=2)
                ax_cdf.axvline(bins[cidx_0][-1], color="orange", ls=":", zorder=2)

                ax_cdf.set_xlabel("δ")
                ax_cdf.set_ylabel("CDF")
                ax_cdf.legend(frameon=False)

                fig.savefig(f"pdf_plots/pdf_cdf_{realisation_folder_number}_{cut_name}_{R}.png", dpi=150, bbox_inches="tight")
                plt.close(fig)
        ...
        for pm, param_p_m in enumerate(PARAMETERS_PM):
            for r_idx, R in enumerate(scale_numbers):
                try:
                    ...
                    print("K_N z={}, bt={}:\n\t".format(redshift, cut_name), k_n_R_z)
    ...
    *, 
    ...
    dataset_identifier_str = "".join(
        [
            ...
            "_cdfpdfcut" if PER_PDF_CDF_CUT else ""
        ]
    )
