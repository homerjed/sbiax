...
# -------------------------------
# Stage flags + helpers (NEW)
# -------------------------------
# Accept CLI flags and/or env vars.
# CLI: --run-nn --run-sbi --run-multi-z --run-figure-one --run-figure-two --run-figure-coverages
# ENV: RUN_NN, RUN_SBI, RUN_MULTI_Z, RUN_FIGURE_ONE, RUN_FIGURE_TWO, RUN_FIGURE_COVERAGES
RUN_NN="${RUN_NN:-}"
RUN_SBI="${RUN_SBI:-}"
RUN_MULTI_Z="${RUN_MULTI_Z:-}"
RUN_FIGURE_ONE="${RUN_FIGURE_ONE:-}"
RUN_FIGURE_TWO="${RUN_FIGURE_TWO:-}"
RUN_FIGURE_COVERAGES="${RUN_FIGURE_COVERAGES:-}"

for arg in "$@"; do
  case "$arg" in
    --run-nn)                 RUN_NN=1 ;;
    --run-sbi)                RUN_SBI=1 ;;
    --run-multi-z)            RUN_MULTI_Z=1 ;;
    --run-figure-one)         RUN_FIGURE_ONE=1 ;;
    --run-figure-two)         RUN_FIGURE_TWO=1 ;;
    --run-figure-coverages)   RUN_FIGURE_COVERAGES=1 ;;
  esac
done

_truthy() { [[ "${1,,}" =~ ^(1|true|yes|y)$ ]]; }

_any_flag_set=false
for v in "$RUN_NN" "$RUN_SBI" "$RUN_MULTI_Z" "$RUN_FIGURE_ONE" "$RUN_FIGURE_TWO" "$RUN_FIGURE_COVERAGES"; do
  if _truthy "$v"; then _any_flag_set=true; break; fi
done

if $_any_flag_set; then
  SEL_NN=$(_truthy "$RUN_NN"               && echo 1 || echo 0)
  SEL_SBI=$(_truthy "$RUN_SBI"             && echo 1 || echo 0)
  SEL_MULTI_Z=$(_truthy "$RUN_MULTI_Z"     && echo 1 || echo 0)
  SEL_FIG1=$(_truthy "$RUN_FIGURE_ONE"     && echo 1 || echo 0)
  SEL_FIG2=$(_truthy "$RUN_FIGURE_TWO"     && echo 1 || echo 0)
  SEL_COV=$(_truthy "$RUN_FIGURE_COVERAGES"&& echo 1 || echo 0)
else
  # Default: run everything
  SEL_NN=1; SEL_SBI=1; SEL_MULTI_Z=1; SEL_FIG1=1; SEL_FIG2=1; SEL_COV=1
fi
...
should_run() {
  case "$1" in
    nn)           [[ "$SEL_NN"    -eq 1 ]];;
    sbi)          [[ "$SEL_SBI"   -eq 1 ]];;
    multi_z)      [[ "$SEL_MULTI_Z" -eq 1 ]];;
    figure_one)   [[ "$SEL_FIG1"  -eq 1 ]];;
    figure_two)   [[ "$SEL_FIG2"  -eq 1 ]];;
    coverages)    [[ "$SEL_COV"   -eq 1 ]];;
    *) echo "Unknown stage '$1'"; return 1;;
  esac
}

# Safe colon-join (skips empty tokens)
deps_join() {
  local out="" tok
  for tok in "$@"; do
    [[ -n "$tok" ]] || continue
...
ONLY_RUN_FIGURES="${3:-false}" # Only run figure_one.py jobs (legacy; superseded by flags)

RUN_LINEARISED=true
RUN_NONLINEAR=true # Run only non-linearised for now (we know it is not biased)

DEFAULT_NDE_TYPE="CNF" # "CNF"
DEFAULT_N_NDES=1
...
if [[ "$SINGLE_RUN" == "true" ]]; then
    ...
    START_SEED=0
    N_SEEDS_GLOBAL=0 # 9 # Number of repeated trainings for SBI
    END_SEED=$(( $START_SEED + $N_SEEDS - 1 ))
    N_PARALLEL=100
fi
        ...
        for PRETRAIN_FLAG in "--pre-train" "--no-pre-train"; do
            ...
            for order_idx_args in "${order_idxs[@]}"; do
                ...
                for bt in "bulk" "tails"; do
                    ...
                    sbi_job_ids=()
                    for z in "${all_redshifts[@]}"; do
                        echo ">>Running NN with z=$z, bulk/tails=$bt, cumulants=$order_idx_args, linearised=$LINEARISED_FLAG"
...
END
                        ...
                        if should_run sbi; then
                            echo ">>Running SBI with z=$z, bulk/tails=$bt, cumulants=$order_idx_args, linearised=$LINEARISED_FLAG"
...
--bulk_or_tails $bt"
                            ...
                            sbi_dep_line="$(deps_join "$data_dep_string" "$nn_job_id")"
...
#SBATCH --mail-user=jed.homer@physik.lmu.de
#SBATCH --mail-type=$MAIL_TYPE
#SBATCH --dependency=afterok:$sbi_dep_line
#SBATCH --export=ALL
...
--redshifts $redshifts_str"
                        ...
                        multi_z_dep_line="$(deps_join "$data_dep_string" "$sbi_deps")"
...
#SBATCH --mail-user=jed.homer@physik.lmu.de
#SBATCH --mail-type=$MAIL_TYPE
#SBATCH --dependency=afterok:$multi_z_dep_line
#SBATCH --export=ALL
...
--redshifts $redshifts_str"
                    ...
                    fig1_dep_line="$(deps_join "$data_dep_string" "$sbi_deps_all" "$multi_z_deps")"
...
#SBATCH --mail-user=jed.homer@physik.lmu.de
#SBATCH --mail-type=$MAIL_TYPE
#SBATCH --dependency=afterok:$fig1_dep_line
#SBATCH --export=ALL
...
END
                    ...
                    echo ">>Submitting Figure One for global_seed=$global_seed (depends on BOTH bulk & tails multi_z if scheduled)"
...
if should_run figure_two; then
  # We can (optionally) make Fig2 wait for all Fig1 jobs.
  ...
  for order_idx_args in "${order_idxs[@]}"; do
      ...
      if [[ "$REQUIRE_FIG1_FOR_FIG2" == "true" && -n "$figure_one_deps_all" ]]; then
          dep_line="$(deps_join "$dep_line" "$figure_one_deps_all")"
      fi
...
if should_run coverages; then
  ...
  for order_idx_args in "${order_idxs[@]}"; do
      ...
      cov_dep_line="$(deps_join "$figure_two_deps")"
...
#SBATCH --mail-user=jed.homer@physik.lmu.de
#SBATCH --mail-type=$MAIL_TYPE
#SBATCH --dependency=afterok:$cov_dep_line
#SBATCH --export=ALL
