...
N_GB=8
N_CPU=8
FIXED_SEED=0 # Seed for which likelihood models are trained at
...
for PRETRAIN_FLAG in "--pre-train" "--no-pre-train"; do
    ...
    # Don't run linearised training with pre-training
    ...
    for bt in "bulk" "tails"; do
        ...
        for order_idx_args in "${order_idxs[@]}"; do
            ...
            echo ">>Running config with cumulants=$order_idx_args, pretrain=$PRETRAIN_FLAG, linearised=$LINEARISED_FLAG, bt=$bt"
...
$FREEZE_FLAG"
                ...
                job_script=$(cat <<END
#!/bin/bash
#SBATCH --job-name=c_sbiz_z${z}_${bt_flag}
#SBATCH --output=$OUT_DIR/sbi_${bt_flag}_fixed.out
#SBATCH --error=$OUT_DIR/sbi_${bt_flag}_fixed.err
...
echo "Running initial sbi.py with seed $FIXED_SEED"
$cmd1
END
)
                    # sbi_job_id=$(echo "$job_script" | sbatch | awk '{print $4}')
                    # all_job_ids+=("$sbi_job_id")
