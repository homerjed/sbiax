...
def get_imnn_compressor(
    ...
    nn_path = os.path.join(results_dir, "imnn.eqx")

    if train:
        net, metrics = train_imnn(
            key,
            net,
            dataset,
            batch_size=1000,
            patience=500,
            min_train_epochs=200
            # config.nn.train.n_batch
        )

        log_F_true = jnp.linalg.slogdet(jnp.linalg.inv(dataset.Finv))
        ...
        plt.ylabel("log |F|")
        plt.savefig(os.path.join(results_dir, "imnn_losses.png"))
        plt.close()
...
if __name__ == "__main__":
    ...
    compression_fn = lambda d, p: net(d)
