...
# class WhiteningScaler(eqx.Module):
#     """
#     A scaling module for inputs to NDE models. Easier to keep track of scaling/rescaling
#     this way.
...
#         *,
#         x_mu_std: Tuple[Float[Array, "x"], Float[Array, "x x"]] = None,
#         q_mu_std: Tuple[Float[Array, "q"], Float[Array, "q q"]] = None,
#         use_scaling=True
#     ):
#         """
#         Initializes the Scaler module.
...
#             self.x_dim = X.shape[-1]
#             self.mu_x = X.mean(axis=0)
#             self.cov_x = jnp.cov(X, rowvar=False)

#         assert not (Q is not None and q_mu_std)
#         if Q is not None:
#             self.q_dim = Q.shape[-1]
#             self.mu_q = Q.mean(axis=0)
#             self.cov_q = jnp.cov(Q, rowvar=False)

#         self.use_scaling = use_scaling

#     def whiten(self, X, mu, cov, eps=1e-8):
#         X_centered = X - mu
#         eigvals, eigvecs = jnp.linalg.eigh(cov)
#         W = eigvecs @ jnp.diag(1.0 / jnp.sqrt(eigvals + eps)) @ eigvecs.T
#         return X_centered @ W

#     def unwhiten(self, X, mu, cov, eps=1e-8):
#         X_centered = X - mu
#         eigvals, eigvecs = jnp.linalg.eigh(cov)
#         W = eigvecs @ jnp.diag(1.0 / jnp.sqrt(eigvals + eps)) @ eigvecs.T
#         return X_centered @ W
